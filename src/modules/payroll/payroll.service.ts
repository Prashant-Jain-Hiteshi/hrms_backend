...
export class PayrollService {
  ...
  async create(createPayrollDto: CreatePayrollDto) {
    const employee = await this.employeeModel.findByPk(createPayrollDto.employeeId);
    if (!employee) {
  ...
  async update(id: string, updatePayrollDto: UpdatePayrollDto) {
    ...
    if (updatePayrollDto.employeeId) {
      const employee = await this.employeeModel.findByPk(updatePayrollDto.employeeId);
      if (!employee) {
        throw new NotFoundException('Employee not found');
      }
    }
  ...
  async processPayroll(employeeIds: string[], periodStart: Date, periodEnd: Date) {
